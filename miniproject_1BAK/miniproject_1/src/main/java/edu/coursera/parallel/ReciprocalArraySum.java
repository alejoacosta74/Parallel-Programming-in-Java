...
public final class ReciprocalArraySum {
    ...
    protected static double parManyTaskArraySum(final double[] input,
            final int numTasks) {
        double sum = 0;
        System.setProperty("java.util.concurrent.ForkJoinPool.common.paralellism","8");
        ...
        for (int i=0; i< numTasks; i++) {
        	...
        	int hi=getChunkEndExclusive(i,numTasks,input.length);
           	ReciprocalArraySumTask t = new ReciprocalArraySumTask(lo,hi, input);
           	taskList.add(t);
        }
        ForkJoinTask.invokeAll(taskList);
        for (int i = 0; i < numTasks; i++) {
			sum= sum + taskList.get(i).getValue();
        }
