...
public final class OneDimAveragingPhaser {
    ...
    public static void runParallelFuzzyBarrier( final int iterations,
        ...
        // array of phasers for point-to-point synchronisation
    	    	
    	Phaser[] phArray = new Phaser[tasks];
    	        
    	Thread[] threads = new Thread[tasks];

        for (int ii = 0; ii < tasks; ii++) {
            final int i = ii;
            phArray[ii]=new Phaser(0);

            threads[ii] = new Thread(() -> {
                double[] threadPrivateMyVal = myVal;
                double[] threadPrivateMyNew = myNew;
                //initialization and registration of phaser object ii within the thread ii
 ...
 /*               	
   ...
   */                  
                    ...
                    //                    boundary conditions for await&advance
             		
                	phArray[i].arrive();
                	
        			if (i==0) {
                   	 phArray[i+1].awaitAdvance(iter);
                    }
                    if (i==(tasks-1)) {
                   	 phArray[i-1].awaitAdvance(iter);
                    }
